<template>
  <div>
    ...
    <ModalComponent :isVisible="isModalVisible" @close="isModalVisible = false">
      <br>
      <h3 class="inline-block bg-blue-500 text-white px-4 py-2 rounded-full focus:outline-none focus:ring-2 focus:ring-opacity-75 cursor-pointer">{{ editMode ? 'Editar Ocorrência' : 'Registrar Ocorrência' }}</h3>
      <form @submit.prevent="submitOccurrence">
        ...
        <br>
        <button type="submit" class="inline-block bg-orange-500 text-white px-4 py-2 rounded-full hover:bg-orange-600 focus:outline-none focus:ring-2 focus:ring-orange-400 focus:ring-opacity-75 cursor-pointer">{{ editMode ? 'Salvar Alterações' : 'Salvar' }}</button>
      </form>
    </ModalComponent>
  </div>
</template>
  ...
  methods: {
    ...
    async fetchMarkers() {
      ...
      if (response.ok) {
        ...
        console.error('Failed to load markers.'); // Alterado para console.error para remover a mensagem de alerta
      }
    },
